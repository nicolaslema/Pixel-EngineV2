...
- React playground's Configurator gained a "Debug HUD" section (`enabled`/`position`/`showFps`/`showQuality`/`showLoop`/`showCells`/`showRipples`/`showTimeline`, wired to `debugHud` on `PixelGridCanvas`) — previously `debugHud` had no control surface anywhere in the playground, so items 5.6/5.10's `createPortal`-based HUD had no manual way to exercise in a real browser.
- Fixed `scripts/smoke-consumer.mjs`'s CI export checks for `PixelGridCanvas`/`PixelCanvas`/`PixelCard`: since item 5.15 wrapped all 4 components in `forwardRef`, they are objects (`$$typeof: Symbol(react.forward_ref)`) at runtime, not functions — `typeof X !== 'function'` was therefore always true and failed CI on every push after 5.15 landed. Now checks for a non-null export instead of a specific `typeof`. `usePixelGridEffect`'s check is unaffected (hooks are plain functions).

### Added
- `PixelCanvas`/`PixelGridCanvas` (and therefore `PixelSurface`/`PixelCard`) now warn in dev (`console.warn`, gated on `NODE_ENV !== "production"`) when `style.width`/`style.height` looks responsive (a percentage or viewport unit, e.g. `"100%"`/`"50vh"`) but `fitMode` isn't `"client"`. With the default `fitMode="none"`, `Canvas2DRenderer.resize()` sets `canvas.style.width`/`height` to a fixed `px` value imperatively on every resize (including at mount), so that responsive style is silently clobbered and never actually takes effect — this was previously an undocumented footgun with no signal that anything was wrong. New pure `getResponsiveCanvasStyleWarning()` (`packages/react/src/canvas-responsive-warning.ts`) for the detection logic. README/API.md document the correct pattern (`fitMode="client"` + `resizeMode`). Non-breaking — dev-only diagnostic, no runtime behavior change.
- `@pixel-engine/react`'s published bundle (`packages/react/dist/index.js`/`index.mjs`) now carries a `"use client"` directive, injected via `tsup.config.ts`'s `banner` option (the package builds to a single file per format — `splitting: false` — so this is the one place that needs it). Every export in the package uses hooks/refs/canvas and is inherently client-only; without the directive, importing anything from `@pixel-engine/react` into a Next.js App Router Server Component tree failed with Next's standard "needs useState... only works in a Client Component" build error, and consumers had to know to wrap it themselves. New `npm run check:use-client` (`scripts/check-use-client-directive.mjs`, wired into CI right after the packages build) asserts the directive is literally the first line of both build outputs, so this can't silently regress if the `banner` config is ever removed. Non-breaking, additive — a no-op string statement in any non-RSC bundler (Vite/CRA/webpack), so existing consumers are unaffected.
...
### Changed
- `packages/react/package.json` now declares `@pixel-engine/core`/`@pixel-engine/effects` as regular `dependencies` (still `^1.0.0`) instead of `peerDependencies`. They're internal dependencies `@pixel-engine/react` genuinely requires to function — not "bring your own version" concerns like `react`/`react-dom` (which correctly stay `peerDependencies`, to avoid duplicate React instances) — and the rest of the codebase already treated them that way (`@pixel-engine/effects` depends on `@pixel-engine/core` via `dependencies`; the root aggregate package does too). Previously, `npm install @pixel-engine/react` alone left a consumer with an unmet-peer warning instead of a working install; now `core`/`effects` install automatically. Non-breaking — anyone already installing all three explicitly is unaffected.
...
- `API.md`: new "`influenceOptions` reference" section clarifying exactly what `PixelGridInfluenceOptions`'s `ripple`/`hover`/`organic` flags each control, since they don't map 1:1 onto `gridConfig`'s per-effect `enabled` fields the way their flat, no-`enabled`-wrapper shape might suggest — `hover` has no `hoverEffects.enabled` counterpart at all (it's the sole switch), `ripple` is a broader master switch than `gridConfig.rippleEffects.enabled` (which only gates the automatic reactive-ripple-on-interaction pass, not manual `triggerRipple()` calls), and `organic` intentionally OR's with `gridConfig.organicNoise.enabled` (a deliberate design from the organic-noise work, not a duplicate to clean up). Investigated as part of auditing this API's shape consistency; found the actual behavior more nuanced than initially assumed, so documented the real relationships instead of a code change.
- Full JSDoc pass on every public nested option type in `packages/react/src/types.ts` (`ScrollReactiveGridOptions`, `SectionTransitionOptions`, `ThemeSyncOptions`, `DebugHudOptions`, `SsrPlaceholderOptions`, `UsePixelEngineOptions`, `UsePixelGridEffectOptions`, `PixelGridCanvasProps`, mask input types, etc.) and `packages/effects/src/entities/pixel-grid/types.ts` (`HoverEffectsOptions`/`HoverMagneticOptions`, `RippleEffectsOptions`, `BreathingOptions`, `AutoMorphOptions`, `OrganicNoiseOptions`, the mask-timeline option types, `PerformanceOptions`, all 11 post-effect `*EffectOptions` types, `PixelGridConfig`, `PixelGridInfluenceOptions`, etc.) — defaults and behavior notes sourced from `normalizeConfig.ts`/the hook implementations, cross-referenced against `API.md` rather than duplicated prose. The V1 JSDoc pass (item 5.12) covered the 4 components and 2 main hooks but not the option types themselves, so IDE autocomplete previously gave no guidance beyond 1-2 levels of nesting (e.g. `gridConfig.hoverEffects.magnetic.strength` had zero doc comments along the way). `Resolved*` internal variants intentionally left undocumented — they mirror the corresponding `*Options` type's meaning and aren't something a consumer constructs by hand.
- `README.md`: new "Framework setup" subsection with copy-pasteable install + minimal usage snippets for Next.js App Router, Vite, and Create React App — previously the only end-to-end example was `playground/react/`, explicitly a QA harness (resolves against source via a Vite alias), not consumer material. Scoped to snippets only (confirmed with the user); a dedicated `packages/react/README.md` remains for the Etapa 2 docs-structure decision (item 2.1), since that's exactly where the doc architecture question gets settled.
- `API.md`: split the flat `PixelGridCanvas` "web utility options" table into "Common" (`scrollReactive`/`sectionTransition`/`themeSync`/`statePreset`) and "Advanced (accessibility, SSR, debugging)" (`respectReducedMotion`/`ssrPlaceholder`/`debugHud`) subsections, so a newcomer can tell at a glance that `debugHud` is a dev-only diagnostic tool and not a feature on the same footing as `scrollReactive`.
- New `packages/react/README.md`: a complete, React-only guide (Install, Quick Start by framework, Concepts — the `preset`→`gridConfig`→`mask`→`influenceOptions` mental model, full API Reference, Cookbook, Troubleshooting) — resolves the Etapa 2 docs-structure decision (item 2.1, single expanded doc over a multi-page directory, since it's what a consumer sees directly on the package's npmjs.com page). All React-specific content previously duplicated across the root `README.md` and `API.md` moved here as the single source of truth: `API.md` now covers only vanilla `core`/`effects` usage (with a pointer to the new doc), and the root `README.md`'s React sections (Framework setup, Responsive canvas, React Usage by Complexity, React Surface Components) were trimmed to short summaries linking to it.

### Tests
- New `React.StrictMode` mount → unmount → remount coverage for `usePixelEngine` and `usePixelGridEffect` (`usePixelEngine.test.tsx`/`usePixelGridEffect.test.tsx`), confirming with evidence what was previously only true "by inspection": StrictMode's dev-mode double-invoked mount effect creates and fully tears down a "phantom" engine/effect before the surviving instance is created, a genuine unmount tears the surviving instance down too, and a fresh mount cycle after a full unmount works cleanly with no leaked ref/state. For `usePixelGridEffect` specifically, confirmed the grid-creation effect (gated on a stable, non-null `engine`) only creates the `PixelGridEffect` once per real mount, not once per StrictMode phantom pass. Verified both new tests actually catch a regression (temporarily reverted `usePixelEngine`'s `engine.destroy()` call and `usePixelGridEffect`'s `engine.removeEntity()` call, confirmed the new tests failed, then restored them).
